...
function CategoryDetails() {
    ...
    useEffect(()=> {
      if(localStorage.getItem('viewCategory')){
        var view = JSON.parse(localStorage.getItem('viewCategory'))
        setViewData(view)
      }
    }, [])
  return (
        ...
        <SoftBox mb={3}>
          <Card>
            <SoftBox display="flex" justifyContent="space-between" alignItems="center" p={3}>
              <SoftTypography variant="h6">Users Details  {viewData && viewData.title ? (<>({viewData.title})</>) : ""}
            ...
            <SoftButton variant="gradient" color="info" size="small"
                onClick={()=>{
                    localStorage.setItem('viewCategory', JSON.stringify(viewData))
            ...
            <SoftButton style={{marginLeft: "20px"}} variant="primary"  color="info" size="small"
              ...
              >Back To Category List</SoftButton>
            </SoftBox>
          </Card>
        </SoftBox>
      </SoftBox>
